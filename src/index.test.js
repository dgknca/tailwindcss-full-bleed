...
const fullBleedPlugin = require('./full-bleed');

function run(config, plugin = tailwindcss) {
  let { currentTestName } = expect.getState();
  config = {
    ...{ corePlugins: { preflight: false }, plugins: [fullBleedPlugin] },
    ...config,
  };
...
describe('full-bleed plugin', () => {
  it('should add the `bleed-bg` utilities', () => {
    const config = {
      content: [
        { raw: String.raw`<div class="bleed-bg bleed-bg-r bleed-bg-l"></div>` },
      ],
    };
  ...
  it('should override `colors` with `bleedColors`', () => {
    const config = {
      content: [
        { raw: String.raw`<div class="bleed-primary bleed-secondary"></div>` },
      ],
      theme: {
        extend: {
          bleedColors: {
  ...
  it('should add the `bleed-border` utilities', () => {
    ...
    return run(config).then((result) => {
      ...
      .bleed-border {
        ...
        border-top: var(--tw-full-bleed-border-top-width) solid var(--tw-full-bleed-color);
        border-bottom: var(--tw-full-bleed-border-bottom-width) solid var(--tw-full-bleed-color);
      }
      .bleed-border-b {
        --tw-full-bleed-border-bottom-width: 1px;
        border-image: linear-gradient(var(--tw-full-bleed-color) 0 0) 1 / /0 100vw 0 100vw;
  ...
  it('should override `borderWidth` with `bleedBorderWidth`', () => {
    const config = {
      content: [
        { raw: String.raw`<div class="bleed-border-sm bleed-border-lg"></div>` },
      ],
      theme: {
        extend: {
          bleedBorderWidth: {
  ...
  it('should generate arbitrary values', () => {
    const config = {
      content: [
        {
          raw: String.raw`<div class="bleed-[#ff0000] bleed-border-t-[1em]"></div>`,
        },
      ],
    };
